...
const VALID_AI_TONES = ['warm', 'professional', 'friendly', 'poetic'];
const VALID_RESPONSE_LENGTHS = ['short', 'balanced', 'detailed'];
const VALID_ADVICE_STYLES = ['comfort', 'balanced', 'actionable'];
const VALID_PERSONA_PRESETS = ['none', 'gentle_friend', 'calm_coach', 'clear_reflector'];

function validateAiTone(tone) {
  if (tone === undefined || tone === null) {
    return { valid: true };
  }
  if (typeof tone !== 'string' || !VALID_AI_TONES.includes(tone)) {
    return { valid: false, error: 'AI 톤은 warm, professional, friendly, poetic 중 하나여야 합니다.' };
  }
  return { valid: true, value: tone };
}
...
function validatePersonaPreset(preset) {
  ...
  if (typeof preset !== 'string' || !VALID_PERSONA_PRESETS.includes(preset)) {
    return { valid: false, error: '페르소나는 none, gentle_friend, calm_coach, clear_reflector 중 하나여야 합니다.' };
  }
